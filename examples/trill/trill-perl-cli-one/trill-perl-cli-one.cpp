...
public:
    ...
    std::string call_perl_make_note_on (int pitch, int vel) {
        dSP;                      // initialize stack pointer
        ENTER;                    // start new scope
        SAVETMPS;                 // save temporaries

        PUSHMARK(SP);             // prepare stack
        XPUSHs(sv_2mortal(newSViv(pitch)));
        XPUSHs(sv_2mortal(newSViv(vel)));
        PUTBACK;                  // make stack valid

        int count = call_pv("make_note_on", G_SCALAR);  // call perl function
    ...
    std::string call_perl_make_note_off (int pitch) {
        dSP;                      // initialize stack pointer
        ENTER;                    // start new scope
        SAVETMPS;                 // save temporaries
        ...
        int count = call_pv("make_note_off", G_SCALAR);  // call perl function
        SPAGAIN;                  // refresh stack pointer
    ...
    void run() {
        ...
        try {
            while (true) {
                touchSensor.readI2C();
                float sensor_value;
                sensor_value = touchSensor.rawData[0];
                // std::cout << "Sent: " << sensor_value << std::endl;
                if ( (sensor_value > 0.05) && !is_note_on) {
                    is_note_on = true;
                    std::string msg = call_perl_make_note_on(60, 114);
                    send_message(msg);
                }
                else if ( (sensor_value < 0.05) && is_note_on ){
                    is_note_on = false;
                    std::string msg = call_perl_make_note_off(60);
                    send_message(msg);
                }
...
int main(int argc, char* argv[]) {
    const char *my_argv[] = { "", "one-note.pl", NULL };
